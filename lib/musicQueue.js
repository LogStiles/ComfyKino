...
const EMPTY_CHANNEL_TIMEOUT_MS = 60_000; //how long to wait after the channel empties before leaving
...
const songPlayer = async (guild) => {
    ...
    const initialResource = createAudioResource(songQueue.currSong.path, { inlineVolume: true });
    initialResource.volume.setVolumeLogarithmic(songQueue.volume / 100);
    songQueue.player.play(initialResource); //play the song
    ...
    songQueue.player.on(AudioPlayerStatus.Idle, async () => { //when the song is done playing
        if (songQueue.songs.length === 0) { //if there is no song to play
            ...
            clearTimeout(songQueue.emptyChannelTimeout);
        ...
        songQueue.textChannel.send(getNowPlayingPayload(songQueue.currSong));
        const nextResource = createAudioResource(songQueue.currSong.path, { inlineVolume: true });
        nextResource.volume.setVolumeLogarithmic(songQueue.volume / 100);
        songQueue.player.play(nextResource); //play the next song
    });
};
...
//sets the playback volume to an exact percentage (1-100)
const setVolume = (interaction, serverQueue, amount) => {
    if (!interaction.member.voice.channel) {
        return interaction.reply('You need to be in a voice channel first.');
    }
    if (!serverQueue) {
        return interaction.reply('There is no queue to set the volume on.');
    }
    serverQueue.volume = amount;
    if (serverQueue.player.state.status === AudioPlayerStatus.Playing || serverQueue.player.state.status === AudioPlayerStatus.Paused) {
        serverQueue.player.state.resource?.volume?.setVolumeLogarithmic(amount / 100);
    }
    return interaction.reply(`Volume set to ${amount}.`);
};

//adjusts the playback volume by a delta, clamped to 1-100
const adjustVolume = (interaction, serverQueue, delta) => {
    if (!interaction.member.voice.channel) {
        return interaction.reply('You need to be in a voice channel first.');
    }
    if (!serverQueue) {
        return interaction.reply('There is no queue to adjust the volume on.');
    }
    const newVolume = Math.min(100, Math.max(1, serverQueue.volume + delta));
    serverQueue.volume = newVolume;
    if (serverQueue.player.state.status === AudioPlayerStatus.Playing || serverQueue.player.state.status === AudioPlayerStatus.Paused) {
        serverQueue.player.state.resource?.volume?.setVolumeLogarithmic(newVolume / 100);
    }
    return interaction.reply(`Volume adjusted to ${newVolume}.`);
};

//reports the current playback volume
const getVolume = (interaction, serverQueue) => {
    if (!interaction.member.voice.channel) {
        return interaction.reply('You need to be in a voice channel first.');
    }
    if (!serverQueue) {
        return interaction.reply('There is no queue to check the volume of.');
    }
    return interaction.reply(`The current volume is ${serverQueue.volume}.`);
};

//removes a song at the given 1-indexed position from the queue (1 = currently playing)
const removeSong = (interaction, serverQueue, position) => {
    if (!interaction.member.voice.channel) {
        return interaction.reply('You need to be in a voice channel first.');
    }
    if (!serverQueue) {
    ...
    if (position < 1 || position > serverQueue.songs.length) {
        return interaction.reply(`Position ${position} is out of range! The queue currently has ${serverQueue.songs.length} songs.`);
    }

    const songName = getSongName(path.basename(serverQueue.songs[position - 1]));

    if (position === 1) {
        serverQueue.player.stop(); //the Idle handler in songPlayer shifts songs[0] off and advances to the next song
    } else {
        serverQueue.songs.splice(position - 1, 1);
    }

    return interaction.reply(`Removed ${songName} from the queue.`);
};
...
//tears down a guild's queue after its voice channel has been empty past the grace period
const leaveEmptyChannel = (guildId) => {
    const serverQueue = queueMap.get(guildId);
    if (!serverQueue) return;
    if (!serverQueue.currSong) { //still connecting, the connection/subscription aren't ready to tear down yet
        serverQueue.emptyChannelTimeout = setTimeout(() => leaveEmptyChannel(guildId), EMPTY_CHANNEL_TIMEOUT_MS);
        return;
    }
    serverQueue.player.stop();
    serverQueue.songs = [];
    serverQueue.connection.destroy(); //leave the channel
    serverQueue.subscription?.unsubscribe();
    queueMap.delete(guildId); //remove the queue from the queueMap
    serverQueue.textChannel.send('Everyone left the voice channel, so I left too.');
};

//starts/cancels the empty-channel grace timer in response to a VoiceStateUpdate event
const handleVoiceStateUpdate = (oldState, newState) => {
    const guild = oldState.guild ?? newState.guild;
    const serverQueue = queueMap.get(guild.id);
    if (!serverQueue) return; //check if no serverqueue

    const humanMembers = serverQueue.voiceChannel.members.filter(member => !member.user.bot);
    if (humanMembers.size === 0) { //if no people in channel
        if (!serverQueue.emptyChannelTimeout) { //if no timeout set
            serverQueue.emptyChannelTimeout = setTimeout(() => leaveEmptyChannel(guild.id), EMPTY_CHANNEL_TIMEOUT_MS); //set timeout that causes bot to leave channel
        }
    } else if (serverQueue.emptyChannelTimeout) { //if timeout set but people in channel
        clearTimeout(serverQueue.emptyChannelTimeout); //cancel timeout
        serverQueue.emptyChannelTimeout = null;
    }
};
