...
const handleVoiceStateUpdate = (oldState, newState) => {
    ...
    if (humanMembers.size === 0) {
        if (!serverQueue.emptyChannelTimeout) {
            serverQueue.emptyChannelTimeout = setTimeout(() => leaveEmptyChannel(guild.id), EMPTY_CHANNEL_TIMEOUT_MS);
        }
    } else if (serverQueue.emptyChannelTimeout) {
        clearTimeout(serverQueue.emptyChannelTimeout);
